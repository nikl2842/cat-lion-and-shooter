...
public class Lion : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)          //Metode som kaldes når løven fysisk rammer ind i noget
    {
        if (collision.collider.tag == "Horse")                  //Hvis det løven rammer er tagget "Horse"
        {
            ...
            if (foodEaten >= foodRequiredToGiveBirth && grown == true && numberOfLions < maxLions)     //hvis der er spist nok OG grown == true OG antal løver er under maxLions
            {
                foodEaten = 0;
    ...
    IEnumerator GiveBirth()                                     
    {
        ...
        Instantiate(lionObj, this.transform.position + new Vector3(1, 0, 0), Quaternion.identity);             //spanwner en ny løve
    }
